...
class Listify_WP_Job_Manager_Template extends listify_Integration {
	...
	public function template_include( $template ) {
		$this->is_home = is_page_template( 'page-templates/template-home.php' );
	...
	public function post_class( $classes ) {
		...
		$home = is_page_template( 'page-templates/template-home.php' );
	 ...
	 * @return void
	 */
	public static function the_location_formatted() {
		global $post;

		$location = get_the_job_location( $post );

		$output = '<div class="job_listing-location-formatted" itemprop="address" itemscope itemtype="http://schema.org/PostalAddress">';
		$output .= '<a class="google_map_link" href="http://maps.google.com/maps?q=' . urlencode( $location ) .
		'&zoom=14&size=512x512&maptype=roadmap&sensor=false" target="_blank">';

		do_action( 'listify_the_location_formatted_start', $post );

		if ( $post->geolocation_street ) {
			$output .= '<span class="location-street" itemprop="streetAddress">' . $post->geolocation_street . '</span><br />';
		}

		if ( $post->geolocation_city ) {
			$output .= '<span class="location-locality" itemprop="addressLocality">' . $post->geolocation_city . '</span>'; 
		}

		if ( $post->geolocation_state_long ) {
			$output .= ', <span class="location-region" itemprop="addressRegion">' . $post->geolocation_state_long  . '</span>';
		}

		do_action( 'listify_the_location_formatted_end', $post );
	...
	public function add_submit_button() {
		...
		$label = sprintf( esc_attr__( 'Update %s', 'listify' ), listify_theme_mod( 'label-plural' ) );

		if ( is_front_page() ) {
			$label = sprintf( esc_attr__( 'Search %s', 'listify' ), listify_theme_mod( 'label-plural' ) );
		}
	...
	public function get_grid_columns( $style = false ) {
		...
		if ( ( listify_job_listing_archive_has_sidebar() || 'side' == $listify_job_manager->map->position() ) && ! ( is_page_template( 'page-templates/template-home.php' ) || is_page_template( 'page-templates/template-full-width-blank.php' ) ) ) {
